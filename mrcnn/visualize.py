...
def frag_percentage_count(image,  boxes, masks, class_ids, scores, class_names, filter_classs_names=None,
               scores_thresh=0.1, save_dir=None, mode=3):
    """
        image: image array
...
def apply_only_mask(image, mask, color, alpha=0.5):
    """Apply the given mask to the image.
    """
    for c in range(1):
        image[:, :, c] = np.where(mask == 1,
...
def save_mask_result(image,image_name, boxes, masks, class_ids, class_names,
                      scores=None, title="",
    ...
    for i in range(N):
        ...
        if show_mask:
            ...
            masked_image = apply_mask(masked_image, mask, color)#change to black background
            print(masked_image)
            imsave('./result_mask_save/'+image_name,masked_image)
            # masked_image = Image.fromarray(masked_image)
            # masked_image.astype(np.uint8).save('./test2.jpg')
        # Mask Polygon
...
def display_instances(image,image_name, boxes, masks, class_ids, class_names,
    ...
    ax.imshow(masked_image.astype(np.uint8))
    plt.savefig('./result_mask_save/'+image_name)
    if auto_show:
        plt.show()
